...
export default function Home() {
  ...
  if (loading) {
    return (
      <div style={{ display:'flex', alignItems:'center', justifyContent:'center', height:'100vh', fontSize:'1.5rem', color:'#e8d5a3', background:'rgba(0,0,0,0.85)' }}>
        Loading...
      </div>
    )
  }

  // R50k tick = bottom of tube = top of bulb join
  // From container bottom: tube bottom is at BULB_SIZE/2 - 3
  const TUBE_BOTTOM_FROM_BOTTOM = BULB_SIZE / 2 - 3
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'2rem', position:'relative', backgroundImage:'url(/church.jpg)', backgroundSize:'cover', backgroundPosition:'center', backgroundRepeat:'no-repeat' }}>
      ...
      <div style={{ position:'relative', zIndex:1, display:'flex', flexDirection:'column', alignItems:'center' }}>
        <h1 style={{ fontSize:'clamp(1.3rem,3.5vw,2.6rem)', fontWeight:700, color:'#c5a028', marginBottom:'0.2rem', textAlign:'center', letterSpacing:'0.08em', textShadow:'0 2px 8px rgba(0,0,0,0.9)', lineHeight:1.2 }}>
          St Peter Church
        </h1>
        <h2 style={{ fontSize:'clamp(1rem,2.5vw,1.8rem)', fontWeight:600, color:'#e8d5a3', marginBottom:'0.4rem', textAlign:'center', letterSpacing:'0.12em', textShadow:'0 2px 8px rgba(0,0,0,0.9)' }}>
          Organ Fund
        </h2>
        ...
        <div style={{ position:'relative', width:'220px', height:`${TUBE_HEIGHT + BULB_SIZE + 10}px`, margin:'0 auto 2rem' }}>

          {MILESTONES.map(m => {
            const fraction = (m.val - BULB_THRESHOLD) / (GOAL - BULB_THRESHOLD)
            const bottomPx = TUBE_BOTTOM_FROM_BOTTOM + fraction * TUBE_HEIGHT
            const reached = amount >= m.val
            return (
              <div key={m.val} style={{ position:'absolute', bottom:`${bottomPx}px`, left:'calc(50% + 43px)', display:'flex', alignItems:'center', gap:'6px', whiteSpace:'nowrap' }}>
                <div style={{ width:'14px', height:'2px', background: reached ? m.color : 'rgba(255,255,255,0.35)', flexShrink:0 }} />
                <span style={{ fontSize:'0.7rem', fontWeight:500, color: reached ? m.color : 'rgba(255,255,255,0.4)' }}>
                  {m.label}
                </span>
              </div>
            )
          ...
          <div style={{ position:'absolute', top:0, left:'50%', transform:'translateX(-50%)', width:'80px', height:`${TUBE_HEIGHT}px`, border:'3px solid #c5a028', borderRadius:'40px 40px 0 0', overflow:'hidden', background:'rgba(255,255,255,0.07)', boxShadow:'inset 0 0 20px rgba(0,0,0,0.6),0 0 25px rgba(197,160,40,0.3)' }}>
          ...
          <div style={{ position:'absolute', top:`${TUBE_HEIGHT - BULB_SIZE/2 + 3}px`, left:'50%', transform:'translateX(-50%)', width:`${BULB_SIZE}px`, height:`${BULB_SIZE}px`, borderRadius:'50%', border:'3px solid #c5a028', background: bulbFilled ? fillColor : 'rgba(255,255,255,0.05)', transition:'background 0.8s', boxShadow: bulbFilled ? `0 0 25px ${fillColor}90` : 'none' }} />
        </div>
        ...
        <div style={{ textAlign:'center', background:'rgba(0,0,0,0.5)', borderRadius:'14px', padding:'1rem 2.5rem', border:`1px solid ${overGoal ? overColor+'80' : 'rgba(197,160,40,0.3)'}`, boxShadow: overGoal ? `0 0 20px ${overColor}40` : 'none', transition:'border 0.5s,box-shadow 0.5s' }}>
          <div style={{ fontSize:'clamp(1.8rem,5vw,3.5rem)', fontWeight:700, color:fillColor, textShadow:`0 0 30px ${fillColor}60`, letterSpacing:'0.02em', lineHeight:1 }}>
            {formatCurrency(Math.min(amount, GOAL))}
          </div>
          {overGoal && (
            <div style={{ fontSize:'clamp(1rem,3vw,1.8rem)', fontWeight:700, color:overColor, textShadow:`0 0 20px ${overColor}80`, marginTop:'0.3rem', letterSpacing:'0.02em', animation:'pulse 1.5s ease-in-out infinite' }}>
